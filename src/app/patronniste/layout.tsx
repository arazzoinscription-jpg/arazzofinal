        ...
        <div className="px-5 py-5 border-b border-white/10">
          <Link href="/" className="nav-center flex items-center gap-2.5">
            ...
            <div className="nav-label">
              ...
              <div className="font-playfair italic text-orange-300 text-xs">{ui.patronnisteSpace}</div>
            </div>
          </Link>
        </div>
        ...
        <div className="px-4 py-4 border-b border-white/10">
          <div className="nav-center flex items-center gap-3">
            ...
            <div className="min-w-0 nav-label">
              ...
              <div className="text-orange-300 text-xs">{profile?.role === "admin" ? ui.roleAdmin : ui.rolePatronniste}</div>
            </div>
          </div>
        </div>

        <ProSidebar variant="patronniste" lang={lang} />

        <div className="p-3 border-t border-white/10">
          <form action="/api/auth/signout" method="POST">
            <button type="submit" className="nav-center w-full flex items-center gap-3 px-3 py-2 rounded-xl text-white/60 hover:text-white hover:bg-white/10 text-sm font-medium transition-colors">
        ...
        <div className="sticky top-0 z-20 bg-white/85 dark:bg-[#0d0a1c]/85 backdrop-blur-md border-b border-cream-200 dark:border-white/10">
          <div className="px-4 sm:px-6 lg:px-8 py-3 flex items-center gap-3 sm:gap-4">
            <ProMobileNav
              ...
              roleLabel={profile?.role === "admin" ? ui.roleAdmin : ui.rolePatronniste}
              lang={lang}
              brand={ui.patronnisteSpace}
            />
