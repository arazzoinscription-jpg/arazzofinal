...
export default async function DemandesPage() {
  ...
  return (
    <div className="min-h-screen bg-cream-DEFAULT dark:bg-transparent p-8">
      <div className="max-w-5xl mx-auto text-gray-900 dark:text-white">
        ...
        <div className="space-y-3 mb-10">
          {!pending.length ? (
            <div className="rounded-2xl bg-white dark:bg-white/[0.04] border border-cream-200 dark:border-white/10 p-10 text-center text-gray-400">
            ...
            pending.map((r) => {
              ...
              return (
                <div key={r.id} className="rounded-2xl bg-white dark:bg-white/[0.04] border border-cream-200 dark:border-white/10 p-4 flex flex-wrap items-center gap-4">
                  ...
                  {others.map((r) => {
                    ...
                    return (
                      <tr key={r.id}>
                        ...
                        <td className="px-4 py-3 text-gray-500 dark:text-white/50">{ROLE_LABEL[r.requested_role]}</td>
                        <td className="px-4 py-3">
                          <span className={`text-[11px] font-semibold px-2 py-1 rounded-full ${BADGE[r.statut]}`}>{STATUT[r.statut]}</span>
                        </td>
                        <td className="px-4 py-3 text-gray-400 text-end">{new Date(r.created_at).toLocaleDateString("fr-FR")}</td>
                      </tr>
                    );
                  })}
                </tbody>
