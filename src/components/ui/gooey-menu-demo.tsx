...
function GooeyDemo() {
  ...
  return (
    <div className="relative w-full h-full min-h-[600px] flex items-center justify-center dark:bg-black bg-white">
      ...
      <div className="absolute top-4 left-4" style={{ filter: "url(#gooey-filter-menu)" }}>
        {/* Menu Items */}
        <AnimatePresence>
          {isOpen &&
            MENU_ITEMS.map((item, index) => {
              ...
              return (
                <motion.button
                  ...
                  animate={{ y: (index + 1) * 44, opacity: 1 }}
                  exit={{
                    y: 0,
                    opacity: 0,
                    transition: { delay: (MENU_ITEMS.length - index) * 0.05, duration: 0.4, type: "spring", bounce: 0 },
                  }}
                ...
                >
                  <AnimatePresence mode="wait">
                    <motion.div
                      ...
                      transition={{ delay: index * 0.05, duration: 0.2, type: "spring", bounce: 0 }}
                    >
