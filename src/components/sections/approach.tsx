...
function PillarCard({ num, Icon, tint, title, desc, index }: { num: string; Icon: LucideIcon; tint: string; title: string; desc: string; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white dark:bg-white/[0.04] rounded-[1.75rem] p-8 border border-cream-200 dark:border-white/10 shadow-soft transition-colors duration-300"
    >
      <span className="absolute top-6 right-7 font-playfair text-7xl font-bold text-cream-200 dark:text-white/10 select-none group-hover:text-orange-DEFAULT transition-colors duration-300">{num}</span>
      <div className="relative">
        ...
        <h3 className="font-playfair text-2xl font-bold text-gray-900 dark:text-white mb-3">{title}</h3>
        <p className="text-gray-600 dark:text-white/60 leading-relaxed font-dm">{desc}</p>
      </div>
